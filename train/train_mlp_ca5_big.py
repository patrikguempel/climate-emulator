...
def create_model():
    ...
    for i in range(10):
        ...
        normalized_branch = LayerNormalization()(branch_starts[i])
        normalized_other_branches = LayerNormalization()(other_branches)

        query = tf.expand_dims(normalized_branch, axis=1)
        key_value = tf.expand_dims(normalized_other_branches, axis=1)
    ...
    for i in range(10):
        if i < 2:
            hid_0 = Dense(512, activation='relu')(attention_outputs[i])
            hid_1 = Dense(256, activation='relu')(hid_0)
            hid_2 = Dense(256, activation='relu')(hid_1)
            out_hid2 = Dense(128, activation='elu')(hid_2)
            out_lin = Dense(60, activation='linear')(out_hid2)
        else:
            hid_0 = Dense(256, activation='relu')(attention_outputs[i])
            hid_1 = Dense(256, activation='relu')(hid_0)
            hid_2 = Dense(128, activation='relu')(hid_1)
            out_hid2 = Dense(16, activation='elu')(hid_2)
