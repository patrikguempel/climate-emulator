...
def create_model():
    ...
    for i in range(12):
        ...
        lower_branch_intermediate += [hid_3]

    # 10 linear layers (recombine first 2 branches)
    lower_branch_outputs = []
    for i in range(10):
        if i < 2:
            branch = Concatenate()([lower_branch_intermediate[2*i], lower_branch_intermediate[2*i + 1]])
            out = Dense(60, activation="linear")(branch)
        else:
            out = Dense(1, activation="linear")(lower_branch_intermediate[i+2])
