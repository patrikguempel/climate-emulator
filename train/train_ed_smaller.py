...
intermediate_dim = 231  # define node size of first and last hidden layers of Encoder or Decoder (Behrens2022)
...
ED.compile(ke.optimizers.Adam(lr=0.0001), loss=mse,
...
os.makedirs("models/ed2", exist_ok=True)

callback_lr = LearningRateScheduler(schedule, verbose=1)  # use callback for lr scheduler

csv_logger = CSVLogger('models/ed2/csv_logger.txt')  # define path where history of training is stored
